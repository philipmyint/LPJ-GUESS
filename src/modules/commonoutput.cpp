...
CommonOutput::CommonOutput() {
	// Annual output variables (added clitter aboveground by weichao)
    ...
    declare_parameter("file_clitter_ag", &file_clitter_ag, 300, "Aboveground Litter C output file"); // added by weichao
...
void CommonOutput::define_output_tables() {
	...
	create_output_table(out_clitter_ag,     file_clitter_ag,     clitter_ag_columns); // added by weichao
...
void CommonOutput::outannual(Gridcell& gridcell) {
	...
	int c, m;
	double flux_veg, flux_repr, flux_soil, flux_fire, flux_est, flux_seed, flux_charvest, flux_prescribed;
	double c_fast, c_slow, c_harv_slow;

	double surfsoillitterc,surfsoillittern,cwdc,cwdn,centuryc,centuryn,n_harv_slow,availn;
	double clitter_SurfMetab,clitter_SurfStruct,clitter_SurfFWD,clitter_SurfCWD; // added by weichao
	...
	double landcover_clitter_ag[NLANDCOVERTYPES]={0.0};	// added by weichao
	...
	double mean_standpft_clitter_ag=0.0; // added by weichao
    double mean_standpft_clitter_leaf=0.0; // added by weichao
    double mean_standpft_clitter_sap=0.0; // added by weichao
    double mean_standpft_clitter_heart=0.0; // added by weichao	
	...
	double clitter_ag_gridcell=0.0; // added by weichao
	double clitter_leaf_gridcell=0.0; // added by weichao
	double clitter_sap_gridcell=0.0; // added by weichao
	double clitter_heart_gridcell=0.0; // added by weichao
	...
	double standpft_clitter_ag=0.0; // added by weichao
	...
	while (pftlist.isobj) {
		...
		mean_standpft_clitter_ag=0.0;	// added by weichao
		mean_standpft_clitter_leaf=0.0;	// added by weichao	
		mean_standpft_clitter_sap=0.0;	// added by weichao	
		mean_standpft_clitter_heart=0.0;	// added by weichao			
		...
		while (gc_itr != gridcell.end()) {
			...
			standpft_clitter_ag=0.0; // added by weichao
			...
			while (stand.isobj) {
                ...
                standpft_clitter_ag += patchpft.litter_leaf + patchpft.litter_sap + patchpft.litter_heart; // added by weichao
				...
				standpft_clitter_ag/=(double)stand.npatch(); //added by weichao
				...
				landcover_clitter_ag[stand.landcover]+=standpft_clitter_ag*stand.get_landcover_fraction();// added by weichao
				...
				mean_standpft_clitter_ag += standpft_clitter_ag * stand.get_gridcell_fraction() / active_fraction; // added by weichao
				mean_standpft_clitter_leaf += standpft_clitter_leaf * stand.get_gridcell_fraction() / active_fraction; // added by weichao
				mean_standpft_clitter_sap += standpft_clitter_sap * stand.get_gridcell_fraction() / active_fraction; // added by weichao
				mean_standpft_clitter_heart += standpft_clitter_heart * stand.get_gridcell_fraction() / active_fraction; // added by weichao
				...
				clitter_ag_gridcell+=standpft_clitter_ag*fraction_of_gridcell; // added by weichao
				clitter_leaf_gridcell+=standpft_clitter_leaf*fraction_of_gridcell; // added by weichao
				clitter_sap_gridcell+=standpft_clitter_sap*fraction_of_gridcell; // added by weichao
				clitter_heart_gridcell+=standpft_clitter_heart*fraction_of_gridcell; // added by weichao
		...
		//outlimit(out,out_clitter_ag,   mean_standpft_clitter_ag); // added by weichao
	...
	flux_veg = flux_repr = flux_soil = flux_fire = flux_est = flux_seed = flux_charvest = flux_prescribed = 0.0;
	...
	clitter_SurfMetab = clitter_SurfStruct = clitter_SurfFWD = clitter_SurfCWD = 0.0; // added by weichao
	...
	while (gc_itr != gridcell.end()) {
		...
		while (stand.isobj) {
			...
			flux_prescribed+=patch.fluxes.get_annual_flux(Fluxes::PRESCRIBED_FIRE)*to_gridcell_average;
			...
			for (int r = 0; r < NSOMPOOL-1; r++) {
				...
				if(r == SURFMETA){
					clitter_SurfMetab += patch.soil.sompool[r].cmass * to_gridcell_average;
				}
				else if (r == SURFSTRUCT) {
					clitter_SurfStruct += patch.soil.sompool[r].cmass * to_gridcell_average;
				}
				else if (r == SURFFWD) {
					clitter_SurfFWD += patch.soil.sompool[r].cmass * to_gridcell_average;
				}
				else if (r == SURFCWD) {
					clitter_SurfCWD += patch.soil.sompool[r].cmass * to_gridcell_average;
				}				
			}			
	...
	//outlimit(out,out_clitter_ag,clitter_ag_gridcell); // added by weichao
	...
	if (run_landcover) {
		for(int i=0;i<NLANDCOVERTYPES;i++) {
			if(run[i]) {
				...
				//outlimit(out,out_clitter_ag, landcover_clitter_ag[i]); // added by weichao
	...
	if (run_landcover) {
			...
			outlimit(out,out_cflux, flux_prescribed);
	...
	outlimit(out,out_cflux, flux_veg - flux_repr + flux_soil + flux_fire + flux_est + c_org_leach_gridcell +
			flux_seed + flux_charvest + flux_prescribed + lc.acflux_landuse_change + lc.acflux_harvest_slow);
	...
	// Aboveground CLITTERPOOL Write cpool to file

	outlimit(out,out_clitter_ag, clitter_leaf_gridcell);
	outlimit(out,out_clitter_ag, clitter_sap_gridcell);
	outlimit(out,out_clitter_ag, clitter_heart_gridcell);
	outlimit(out,out_clitter_ag, clitter_SurfMetab);
	outlimit(out,out_clitter_ag, clitter_SurfStruct);
	outlimit(out,out_clitter_ag, clitter_SurfFWD);
	outlimit(out,out_clitter_ag, clitter_SurfCWD);	

	double clitter_ag_total = clitter_leaf_gridcell + clitter_sap_gridcell + clitter_heart_gridcell + clitter_SurfMetab + clitter_SurfStruct + clitter_SurfFWD + clitter_SurfCWD;

	outlimit(out,out_clitter_ag, clitter_ag_total);
