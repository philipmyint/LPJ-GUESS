...
// end   
	...
	if (alive) {
		...
		if (i.cmass_debt <= i.cmass_sap + i.cmass_heart) {
			...
			//! if take stem out of ecosysytem and other live removals in dead pools
			bool stem_out_of_ecosystem;
			bool all_out_of_ecosystem;
            stem_out_of_ecosystem = take_stem_out_of_ecosystem;
			all_out_of_ecosystem = take_all_out_of_ecosystem;
			if(stem_out_of_ecosystem == true && all_out_of_ecosystem == false){
			// harvested stem wood
			...
			i.litter_leaf += i.cmass_leaf * frac_cut;
...
//				dprintf("ATTENTION: pft %s: cmass_debt > cmass_heart; difference=%f\n", (char*)pft.name, i.cmass_debt-i.cmass_heart);
			...
			i.litter_sap += to_partition_sap * (1.0 -  harv_eff * stem_frac) * frac_cut;
			i.litter_heart += to_partition_heart * (1.0 - harv_eff * stem_frac) * frac_cut;
			}
			
			else if(all_out_of_ecosystem == true){
			// harvested stem wood
			harvest += harv_eff * (stem_frac + twig_frac) * (i.cmass_sap + i.cmass_heart - i.cmass_debt) * frac_cut;
			...
			// harvested products consumed (oxidised) this year put into acflux_harvest
			i.acflux_harvest += harvest;
			i.acflux_harvest += i.cmass_leaf * frac_cut;
			
			}
			
			//! keep all live removal in dead pools
			else {
...
//				dprintf("ATTENTION: pft %s: cmass_debt > cmass_heart; difference=%f\n", (char*)pft.name, i.cmass_debt-i.cmass_heart);
			...
			i.litter_sap += to_partition_sap * frac_cut;
			i.litter_heart += to_partition_heart * frac_cut;
			}
			//! end of thinning
			
			//! prescribed fire in litter pools. please note we should do prescribed fire in 4 other surface pool in growth.cpp
            // Patch& patch = stand.getobj();
			// patch.soil.sompool[SURFFWD].cmass += i.litter_sap*0.2;
			
			//! end of prescribed fire in litter pools
		...
		// debt larger than existing wood biomass
			...
			if(date.year == 510 && date.day == 0){
			   dprintf("ATTENTION: cmass_debt (%f) > i.cmass_sap (%f) + i.cmass_heart (%f)\n", i.cmass_debt, i.cmass_sap, i.cmass_heart);
			}
			// dprintf("ATTENTION: cmass_debt (%f) > i.cmass_sap (%f) + i.cmass_heart (%f)\n", i.cmass_debt, i.cmass_sap, i.cmass_heart);
//			i.debt_excess += debt_excess * frac_cut;	// debt_excess currently not dealt with during wood harvest
		...
		// revised by weichao guo
		i.nmass_litter_leaf += i.nmass_leaf * (1.0) * frac_cut;
		i.nmass_litter_sap += i.nmass_sap * (1.0) * frac_cut;
		i.nmass_litter_heart += i.nmass_heart * (1.0) * frac_cut;
...
void harvest_wood(Individual& indiv, double frac_cut, double harv_eff, double res_outtake_twig, 
	...
	if(Prescribed_fire == true){
	Patch &patch = indiv.vegetation.patch;
	double prescribed_fire_fraction;
    prescribed_fire_fraction = 1.0 - dead_fuel_remaining;
	double emission_litter = 0.0;
	patch.soil.sompool[SURFHUMUS].cmass += indiv_cp.litter_leaf * prescribed_fire_fraction * 0.05; //  
	patch.soil.sompool[SURFHUMUS].cmass += indiv_cp.litter_sap * prescribed_fire_fraction * 0.15; // 0.15 = 1-0.85 is the proportion of the fuel carbon burnt emitted into the atmosphere from M.Possell et al., Biogeosciences, 2015
	patch.soil.sompool[SURFHUMUS].cmass += indiv_cp.litter_heart * prescribed_fire_fraction * 0.15; // 0.15 = 1-0.85 is the proportion of the fuel carbon burnt emitted into the atmosphere from M.Possell et al., Biogeosciences, 2015
	
	emission_litter += indiv_cp.litter_leaf * prescribed_fire_fraction * 0.95; //
	emission_litter += indiv_cp.litter_sap * prescribed_fire_fraction * 0.85; //
	emission_litter += indiv_cp.litter_heart * prescribed_fire_fraction * 0.85; //
	patch.fluxes.report_flux(Fluxes::PRESCRIBED_FIRE, emission_litter);	// Put into gridcell.acflux_landuse_change instead at land use change
	}
    // need to create acflux_prescribed	
	//! end of prescribed fire in litter pools
...
double cut_fraction(Patch& patch) {
	...
	int First_cutyear;
    First_cutyear = nyear_spinup + first_cutyear - first_sim_year; // Simulation year when forestry harvesting starts; default is directly after spinup.
...
//		Cut_int=30-(int)(15.0*(bonitet-minbon)/(maxbon-minbon));
		...
		// cut more in first year (added by weichao)
		// if (date.year == First_cutyear) // changed by weichao		
			// cut_fraction = 0.2;
		//if (!((date.year - First_cutyear - patch_order) % Cut_int)) // rule needs to be corrected
		if (!((date.year - First_cutyear) % Cut_int)) // changed by weichao		
			cut_fraction = 0.9999;
	}
...
void harvest_forest(Individual& indiv, Pft& pft, bool alive, double anpp, bool& killed) {
	...
	int First_cutyear = nyear_spinup + first_cutyear - first_sim_year; // Simulation year when forestry harvesting starts; default is directly after spinup.
	if (pft.lifeform==TREE && man_strength > 0.00) {

		double diam = pow(indiv.height / indiv.pft.k_allom2, 1.0 / indiv.pft.k_allom3);
//added by weichao
// dprintf("Year = %d  Month  = %d  Day = %d  Patch.Id = %d  Diam = %f  Height = %f\n", date.year,date.month,date.day,patch.id, diam,indiv.height);
...
// end
		...
		// harvest tree
		else if (indiv.pft.crownarea_max > 10.0){

			// double diam_limit=0.13+0.07*(bonitet-minbon)/(maxbon-minbon); // Harvest of trees > 19 cm
			// double diam_max = diam_limit * 2.0;
			double diam_limit_min;
			double diam_limit_max;
			bool Thinning_only_once;
			diam_limit_min = diam_min * 0.0254; // revised by weichao
			diam_limit_max = diam_max * 0.0254; // revised by weichao
			Thinning_only_once = thinning_only_once;
			
            // double total_cmass_before;
			// double total_cmass_after; 
			// total_cmass_after = total_cmass_before = 0.0; 
			// total_cmass_before = indiv.ccont(); 
			if((diam < diam_limit_max) && (diam > diam_limit_min) && (Thinning_only_once == false)){
					man_strength = 0.9999;
			...
			else if(diam < diam_limit_max && diam > diam_limit_min && (Thinning_only_once == true) && (date.year == First_cutyear)){
					man_strength = 0.9999;
					harvest_wood(indiv, man_strength, indiv.pft.harv_eff, indiv.pft.res_outtake); // frac_cut=man_strength, harv_eff=pft.harv_eff, res_outtake_twig=pft.res_outtake, res_outtake_coarse_root=0	
				}
			//! this just for thinning in first year
			// if(date.year == First_cutyear){
				// if(diam < diam_limit){
					// man_strength = 0.9999;
					// harvest_wood(indiv, man_strength, indiv.pft.harv_eff, indiv.pft.res_outtake); // frac_cut=man_strength, harv_eff=pft.harv_eff, res_outtake_twig=pft.res_outtake, res_outtake_coarse_root=0	
			...
			// else {
				...
				// }
			// }
			//! end of setting for thinning in first year
			
		}
		else if(indiv.pft.crownarea_max <= 10.0){
			double Shrub_remaining;
			Shrub_remaining = shrub_remaining_frac;
			man_strength = 1.0 - Shrub_remaining;
			harvest_wood(indiv, man_strength, indiv.pft.harv_eff, indiv.pft.res_outtake); // haverst shrub
		}
