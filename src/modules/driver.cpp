...
void soilparameters(Soiltype& soiltype, int soilcode, double SOILDEPTH_UPPER, double SOILDEPTH_LOWER) {
	...
	double data[12][9] = {
		...
		{   7.036, 0.128,   0.2, 0.800,   0.4,	0.070,	0.395,	0.92,	0.03},    // 1	Sand
		{   4.767, 0.149,   0.2, 0.770,   0.4,	0.075,	0.441,	0.82,	0.06},    // 2	Loamy Sand
		{   2.495, 0.136,   0.2, 0.740,   0.4,	0.114,	0.435,	0.58,	0.10},    // 3	Sandy Loam
		//{   1.960, 0.138,   0.2, 0.653,   0.4,	0.179,	0.485,	0.17,	0.13},    // 4	Silt Loam
		{   1.960, 0.228,   0.2, 0.653,   0.4, 0.139,	0.485,	0.17,	0.13},    // 4	Silt Loam (the best_et right now)
		{   1.988, 0.133,   0.2, 0.625,   0.4,	0.203,	0.519,	0.10,	0.05},    // 5	Silt
		{   1.932, 0.143,   0.2, 0.680,   0.4,	0.155,	0.451,	0.43,	0.18},    // 6	Loam
		//{   1.960, 0.328,   0.2, 0.653,   0.4,	0.139,	0.485,	0.17,	0.13},    // 4	Silt Loam
		//{   1.988, 0.333,   0.2, 0.625,   0.4,	0.203,	0.519,	0.10,	0.05},    // 5	Silt
		//{   1.932, 0.343,   0.2, 0.680,   0.4,	0.155,	0.451,	0.43,	0.18},    // 6	Loam
		{   1.917, 0.128,   0.2, 0.650,   0.4,	0.175,	0.420,	0.58,	0.27},    // 7	Sandy Clay Loam
		{   0.692, 0.139,   0.2, 0.620,   0.4,	0.218,	0.477,	0.10,	0.34},    // 8	Silty Clay Loam
		{   0.920, 0.113,   0.2, 0.590,   0.4,	0.250,	0.476,	0.32,	0.34},    // 9	Clay Loam
		{   1.095, 0.124,   0.2, 0.560,   0.4,	0.219,	0.426,	0.52,	0.42},    // 10	Sandy Clay
		{   1.054, 0.126,   0.2, 0.530,   0.4,	0.283,	0.492,	0.06,	0.47},    // 11	Silty Clay
		{   0.935, 0.114,   0.2, 0.500,   0.4,	0.286,	0.482,	0.22,	0.58}     // 12	Clay
		
		//{   46.578, 0.105,   0.2, 0.800,   0.4,	0.070,	0.395,	0.92,	0.03},    // 1	Sand
		//{   21.023, 0.119,   0.2, 0.650,   0.4,	0.075,	0.441,	0.82,	0.06},    // 2	Loamy Sand
		//{   55.1, 0.136,   0.2, 0.500,   0.4,	0.114,	0.435,	0.58,	0.10},    // 3	Sandy Loam
		//{   335.349, 0.191,   0.2, 0.500,   0.4,	0.179,	0.485,	0.17,	0.13},    // 4	Silt Loam
		//{   335.149, 0.191,   0.2, 0.500,   0.4,	0.179,	0.485,	0.10,	0.05},    // 5	Silt
		//{   91.631, 0.160,   0.2, 0.650,   0.4,	0.155,	0.451,	0.43,	0.18},    // 6	Loam
		//{   29.26, 0.125,   0.2, 0.575,   0.4,	0.175,	0.420,	0.58,	0.27},    // 7	Sandy Clay Loam
		//{   12.48, 0.140,   0.2, 0.650,   0.4,	0.218,	0.477,	0.10,	0.34},    // 8	Silty Clay Loam
		//{   72.858, 0.142,   0.1, 0.600,   0.4,	0.250,	0.476,	0.32,	0.34},    // 9	Clay Loam
		//{   2.589, 0.098,   0.2, 0.500,   0.4,	0.219,	0.426,	0.52,	0.42},    // 10	Sandy Clay
		//{   16.866, 0.126,   0.1, 0.500,   0.4,	0.283,	0.492,	0.06,	0.47},    // 11	Silty Clay
		//{   14.523, 0.115,   0.2, 0.500,   0.4,	0.286,	0.482,	0.22,	0.58}     // 12	Clay
	};
...
void interp_climate(double mtemp[12], double mprec[12], double msun[12],
	...
	// mwind = mean monthly wind speed (m s-1)
	// mtmin = mean monthly minimum temperature (deg C)
	// mtmax = mean monthly maximum temperature (deg C)
	...
	// dwind = mean daily interpolated wind speed for this year (m s-1)
	// dtmin = mean daily minimum temperature for this year (deg C)
	// dtmax = mean daily maximum temperature for this year (deg C)
	// dNI[365]	  Nesterov fuel dryness Index (degC^2) for the day //@KE added for LMFire

	Date date; // Date object used for interpolation (local to this function)
	double nday, dayct;
	int thismonth, lastmonth, m;
	double mprec_daily[12];
	double prob[365]; //@KE added for random probability generation
	double sumprob;
	...
	while (date.year == 0) { // Date object used for interpolation (local to this function)
		if (date.day == date.middaymonth[date.month]) {
			if (date.month == 11) // December
				nday = (double) (date.middaymonth[0] + 365
						- date.middaymonth[11]);
			else
		...
		dtemp[date.day] = (mtemp[thismonth] - mtemp[lastmonth]) / nday * dayct
				+ mtemp[lastmonth];
		dsun[date.day] = (msun[thismonth] - msun[lastmonth]) / nday * dayct
				+ msun[lastmonth];
		dprec[date.day] = (mprec_daily[thismonth] - mprec_daily[lastmonth])
				/ nday * dayct + mprec_daily[lastmonth];
// added by weichao


		if ((firemode == LMFIRE) || (firemode == LMFIRE_CF)){
			dwind[date.day] = (mwind[thismonth] - mwind[lastmonth]) / nday * dayct
					+ mwind[lastmonth];
		    dlght[date.day] = (mlght[thismonth] - mlght[lastmonth]) / nday * dayct
				+ mlght[lastmonth];					
			dtmin[date.day] = (mtmin[thismonth] - mtmin[lastmonth]) / nday * dayct
					+ mtmin[lastmonth];
			dtmax[date.day] = (mtmax[thismonth] - mtmax[lastmonth]) / nday * dayct
					+ mtmax[lastmonth];
			...
			if ((dprec[date.day] < 3.0) && (dtemp[date.day] > 0.0)) {
				if(date.day==0){
					dNI[date.day] = 0.0;
				}
				dNI[date.day] = dNI[date.day] + dtmax[date.day] * (dtmax[date.day] - (dtmin[date.day] - 4.0));
			}
			else {
				dNI[date.day] = 0.0;
			}
			//printf("NI[day] %f dtmax %f dtmin %f date.day %d date.year %d \n",dNI[date.day],dtmax[date.day],dtmin[date.day],date.day,date.year);

		} //end if LMFIRE

	//@KE added prints to check dtemp and dprec
	/*dprintf ("\n daily temp \n =");
	 for (int iday=0; iday<365; iday++){
	 dprintf ( "%8.2f", dtemp[iday]);
	 }
	 dprintf ("\n");

	 dprintf ("\n daily prec \n =");
	 for (int iday=0; iday<365; iday++){
	 dprintf ( "%8.2f", dprec[iday]);
	 }
	 dprintf ("\n");*/
...
void dailyaccounting_gridcell(Gridcell& gridcell) {
	...
	if (date.day == 0) {
		...
		// ... initialize gridcellpft variables
		// @KE added for LMFire
		...
		if ((firemode == LMFIRE) || (firemode == LMFIRE_CF)){
			...
			while (pftlist.isobj) {
				...
				gridcellpft.BBlive = 0.0;
				gridcellpft.fkill = 0.0;
				gridcellpft.CKkill = 0.0;
				gridcellpft.taukill = 0.0;
...
void daylengthinsoleet(Climate& climate) {
	...
	//climate.eet = 2 * (s / (s + gamma) / lambda) * (uu * hn + vv * sin(hn)) * K;	// Eqn 26;
	climate.eet = 2 * (s / (s + gamma) / lambda) * (uu * hn + vv * sin(hn)) * K;	// Eqn 26;
}
...
void esat(Climate& climate) {
	...
	double esat; //saturation vapor pressure (Pa)
	double al[9]; //!coefficients for liquid water
	double ai[9]; //!coefficients for ice
	int i;
	...
	if (climate.tmin <= 0.0) {   //!these coefficients are for temperature values in Celcius
		...
		for (i = 0; i < 9; i++ ) {
			esat = esat + al[i] * pow(T,i);
		}
	}

	climate.esat = 100. * esat;
	//dprintf("esat() climate.tmin %f climate.esat %f",climate.tmin,climate.esat);
}
