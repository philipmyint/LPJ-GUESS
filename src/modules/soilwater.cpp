...
void hydrology_lpjf(Patch& patch, Climate& climate, double rain_melt, double perc_base,
	...
	//const double SOILDEPTH_EVAP = 400.0;
	//SOILDEPTH_UPPER2 = 500;
	const double SOILDEPTH_UPPER2 = 500;
	const double SOILDEPTH_EVAP = SOILDEPTH_UPPER2*0.4;
		// depth of sublayer at top of upper soil layer, from which evaporation is
		// possible (NB: must not exceed value of global constant SOILDEPTH_UPPER2)
	...
	const double K_AET_DEPTH = (SOILDEPTH_UPPER2 / SOILDEPTH_EVAP - 1.0) *
		...
		// Equates to 1.3 given SOILDEPTH_EVAP=200 mm, SOILDEPTH_UPPER2=500 mm,
	...
	wcont_evap += (rain_melt-aet_layer[0]*SOILDEPTH_EVAP*K_AET_DEPTH/SOILDEPTH_UPPER2-evap)
		/awc[0];
	...
	double perc = 0.0;
	if (percolate) {
		perc = min(SOILDEPTH_EVAP/SOILDEPTH_UPPER2*perc_base*pow(wcont_evap,perc_exp),
													max_rain_melt);
	}
