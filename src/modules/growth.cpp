...
void turnover(double turnover_leaf, double turnover_root, double turnover_sap,
	...
	double& nmass_heart, double& litter_leaf, double& litter_root, //double & litter_wood, // added by weichao
	...
	if (lifeform == TREE) {
		...
		//			// Branch turnover //@KE added to increase dead fuels for LM_Fire.cpp
		// double turnover_branch = 0.05;
		// turnover = turnover_branch * cmass_sap;
		// cmass_sap -= turnover;
		// if (alive){
		// litter_wood += turnover;}
		// dprintf("litter_wood %f += turnover %f\n",litter_wood,turnover);
...
bool allometry(Individual& indiv) {
	...
	if (indiv.pft.lifeform == TREE) {
        ...
        // if(date.year >= nyear_spinup  && date.year < nyear_spinup+150){	
           // FILE *fptr;
           // fptr = fopen("/home/wguo5/scratch/guessrun/data/Diameter_Height_Cmass.txt","a");	
           // if(fptr == NULL){
              // dprintf("Error!");   
              // exit(1);             
           // }
		   // else if(fptr != NULL){
                ...
                // double litter6;
                // double litter7;
                // litter1 = litter2 = litter3 = litter4 = litter5 = litter6 = litter7 = 0.0; 
                // litter1 = ppft.litter_leaf;
                // litter2 = ppft.litter_sap;
                // litter3 = ppft.litter_heart;
                // litter4 = patch.soil.sompool[SURFMETA].cmass;
                // litter5 = patch.soil.sompool[SURFSTRUCT].cmass;
                // litter6 = patch.soil.sompool[SURFFWD].cmass;
                // litter7 = patch.soil.sompool[SURFCWD].cmass;
				// int Year_calender;
                // Year_calender = date.get_calendar_year();
		        // if(Year_calender >= 1950 && indiv.pft.id >= 0 && indiv.id >= 0 && patch.id >= 0 && patch.managed >= 0 && indiv.age >= 0.0 && diam >= 0.0 && \
				// Year_calender < 9999 && indiv.pft.id < 9999 && indiv.id < 9999 && patch.id < 9999 && patch.managed < 9999 && indiv.age < 9999.0 && diam < 9999.0 && \
				// indiv.height < 9999.0 && indiv.cmass_leaf < 9999.0 && indiv.cmass_sap < 9999.0 && indiv.cmass_heart < 9999.0 && indiv.cmass_root < 9999.0 && \
		           // indiv.height >= 0.0 && indiv.cmass_leaf >= 0.0 && indiv.cmass_sap >= 0.0 && indiv.cmass_heart >= 0.0 && indiv.cmass_root >= 0.0){
		           // fprintf(fptr,"\r\n");
		           // fprintf(fptr,"%-10d %-10d %-10d %-10d %-10d %-10.0f %-10.7f %-10.3f %-10.3f %-10.3f %-10.3f %-10.3f %-10.3f %-10.3f %-10.3f %-10.3f %-10.3f %-10.3f %-10.3f %-10.3f\r\n", \
				   // Year_calender, indiv.pft.id, indiv.id, patch.id, patch.managed, indiv.age, indiv.dnpp, diam, indiv.height, indiv.cmass_leaf, indiv.cmass_sap, indiv.cmass_heart, indiv.cmass_root, \
				   // litter1, litter2, litter3, litter4, litter5, litter6, litter7);
				   // //fclose(fptr);
		         // }
		   // }
		   // fclose(fptr);
...
// end of addition by weichao
		...
		// Crown length (Eqn 7)
		if (indiv.height > 2.0){
			indiv.crownlength = indiv.pft.CLs * indiv.height + indiv.pft.CLi;
			if(indiv.crownlength > indiv.height){
				indiv.crownlength = indiv.height;
			}
		}
		else {
			indiv.crownlength = indiv.height;
		...
		// Bole height (Eqn 7)
		indiv.boleht = indiv.height - indiv.crownlength;
		indiv.boleht = max(indiv.boleht, 0.0);
		// indiv.boleht = 0.0;
...
void growth(Stand& stand, Patch& patch) {
			...
			if (!killed) {
				...
				if(!indiv.has_daily_turnover()) {
					...
					turnover(indiv.pft.turnover_leaf, indiv.pft.turnover_root,
						...
						// patch.pft[indiv.pft.id].litter_wood, // added by weichao
	...
	bool Prescribed_fire;
	Prescribed_fire = prescribed_fire;
    if(patch.managed_this_year && (Prescribed_fire == true)){
		//double litter_total = 0.0;	// added by weichao
	    double patch_acflux_prescribed = 0.0;
	    double fuel_remaining_fc;
        fuel_remaining_fc = dead_fuel_remaining; // remaning dead fraction

        // litter_total = i.litter_leaf + i.litter_sap + i.litter_heart + i.SURFMETA_cmass + i.SURFSTRUCT_cmass + i.SURFFWD_cmass + i.SURFCWD_cmass;
        //litter_total = patch.soil.sompool[SURFMETA].cmass + patch.soil.sompool[SURFSTRUCT].cmass + patch.soil.sompool[SURFFWD].cmass + patch.soil.sompool[SURFCWD].cmass;
		
		patch.soil.sompool[SURFHUMUS].cmass += patch.soil.sompool[SURFMETA].cmass * (1.0 - fuel_remaining_fc) * 0.05; //0.05 = 1-0.95 is the proportion of the fuel carbon burnt emitted into the atmosphere from M.Possell et al., Biogeosciences, 2015
        patch.soil.sompool[SURFHUMUS].cmass += patch.soil.sompool[SURFSTRUCT].cmass * (1.0 - fuel_remaining_fc) * 0.05; //0.05 = 1-0.95 is the proportion of the fuel carbon burnt emitted into the atmosphere from M.Possell et al., Biogeosciences, 2015
		patch.soil.sompool[SURFHUMUS].cmass += patch.soil.sompool[SURFFWD].cmass * (1.0 - fuel_remaining_fc) * 0.15; //0.05 = 1-0.85 is the proportion of the fuel carbon burnt emitted into the atmosphere from M.Possell et al., Biogeosciences, 2015
		patch.soil.sompool[SURFHUMUS].cmass += patch.soil.sompool[SURFCWD].cmass * (1.0 - fuel_remaining_fc) * 0.15; //0.05 = 1-0.85 is the proportion of the fuel carbon burnt emitted into the atmosphere from M.Possell et al., Biogeosciences, 2015

		patch_acflux_prescribed += patch.soil.sompool[SURFMETA].cmass * (1.0 - fuel_remaining_fc) * 0.95;
		patch_acflux_prescribed += patch.soil.sompool[SURFSTRUCT].cmass * (1.0 - fuel_remaining_fc) * 0.95;
		patch_acflux_prescribed += patch.soil.sompool[SURFFWD].cmass * (1.0 - fuel_remaining_fc) * 0.85;
		patch_acflux_prescribed += patch.soil.sompool[SURFCWD].cmass * (1.0 - fuel_remaining_fc) * 0.85;
		
        patch.soil.sompool[SURFMETA].cmass *= fuel_remaining_fc;
        patch.soil.sompool[SURFSTRUCT].cmass *= fuel_remaining_fc;		
        patch.soil.sompool[SURFFWD].cmass *= fuel_remaining_fc;			
        patch.soil.sompool[SURFCWD].cmass *= fuel_remaining_fc;	
		
        patch.fluxes.report_flux(Fluxes::PRESCRIBED_FIRE, patch_acflux_prescribed);	// Put into gridcell.acflux_landuse_change instead at land use change	
	}
