...
// change soil depth values by weichao guo



/// soil upper layer depth (mm)
/* const double SOILDEPTH_UPPER = 1000.0;
/// soil lower layer depth (mm)
const double SOILDEPTH_LOWER = 9000.0; */
...
const int OUTPUT_MAXAGECLASS = 2000; // added by weichao
...
public:
	...
	// min air temperature today (deg C)
	double tmin; // added by weichao
	
	// max air temperature today (deg C)
	double tmax; // added by weichao
	...
	// wind speed today
	double wind; // added by weichao

	///lightning today 
	double lght; // added by weichao
	double dlght;
	// daily interpolated lightning flashes (km-2 day-1)	
	double esat;
	// saturation vapor pressure today (Pa)
	double magdd20;
	// accumulated growing degree days (20-year running mean) @KE added
	double NI;
	// !Nesterov fuel dryness Index (degC^2) for the day
...
public:
    ...
    double acflux_fireveg;
	// annual carbon flux to atmosphere from burnt vegetation only
	...
	enum PerPatchFluxType {
		...
		ESTC,
		/// Flux to atmosphere from take-out woody stem by harvest (kgC/m2)
		HARVESTC,
		//! Flux to atmosphere from prescribed fire(added by weichao)
		PRESCRIBED_FIRE,
...
public:
	...
	// Variables added for use in LMFire - @KE added 2019
	double CLs;
	//crown length slope, for calculating crown length as a function of height
	double CLi;
	//crown length intercept, for calculating crown length as a function of height
	double BTs;
	//bark thickness slope, for calculating bark thickness as a function of diameter
	double BTi;
	//bark thickness intercept, for calculating bark thickness as a function of diameter
...
public:
	...
	// Variables added for use in LMFire - @KE added 2018
	double diameter; 	// plant diameter (m)
	double barkt; 		// bark thickness (cm)
	double crownlength; // crown length (m)
	...
	double P_mCK;		// probability of mortality due to crown kill for each cohort // OUTPUT for tracking crown vs. cambial kill
	double P_mtau; 		// probability of mortality due to cambial damage for each cohort // OUTPUT for tracking crown vs. cambial kill
	double k_latosa_new; // size-dependent tree leaf to sapwood area ratio //@KE added TEST

    double ROS;
	double SH;

	//TESTING
	double ann_kill;    		//!annual total probability of mortality
    ...
    // diameter = barkt = crownlength = CK = P_m = P_mCK = P_mtau = k_latosa_new = ROS = SH = ann_kill = day_kill = day_CKkill = day_taukill = 0.0;
	// bvoc
...
public:
	...
	// double litter_wood;  // added by weichao
...
// Variables added for use in LMFire - @KE added 2018
	int cumfires;
	//!number of fires currently burning
	double afire_frac;
	//!fraction of the gridcell burned
	int anumfires;
	//annual number of fires
	int burndays;
	//annual burndays
	double totburn;
	// total area burned (ha d-1) over the course of a year
	double precsum;
	// precipitation summed up over the course of a year
	double unburneda;
	// !unburned area (ha)
	double sumfdi;
	//!FDI summed up over the course of a year
	double burnedf;
	//Tree biomass killed (but not consumed) by fire
	
	double annual_fire_flux;
    double annual_fire_fuel_flux;
	double annual_fire_veg_flux;
	double annual_killed_veg_flux;
	double annual_fire_flux_confirm;
    double annual_fire_fuel_flux_confirm;
	double annual_fire_veg_flux_confirm;
	
	
	double mnumfires[12];
	// number of fires over the course of a month
	double mtotburn[12];
	// total area burned (ha d-1) over the course of a month
	double rabarf[12];
	// total raw area burned (ha d-1) before limited by patch size
	double mfdi[12];
	//Average FDI over the course of a month
	double mnlig[12];
	//number of lightning caused-ignition events per month
	double ROSf[12];
	// !overall forward rate of spread of fire (m min-1) average over the course of a month
	double ROSb[12];
	// !backward rate of spread of fire (m min-1) average over the course of a month
	double Isurf[12];
	// !surface fire intensity (kW m-1) average over the course of a month
...
public:
	...
	//@KE added output variables for stand structure for out_ncdf_annual 5/2015
	double height_ageclass[OUTPUT_MAXAGECLASS];
	// plant height by age class (cohort/individual mode only; used by function
	// out_ncdf_annual)
	double lai_ageclass[OUTPUT_MAXAGECLASS];
	// LAI by age class (cohort/individual mode only; used by function
	// out_ncdf_annual)
	double age_ageclass[OUTPUT_MAXAGECLASS];
	// age by age class (cohort/individual mode only; used by function
	// out_ncdf_annual)
	double diameter_ageclass[OUTPUT_MAXAGECLASS];
	// diameter of stem by age class (cohort/individual mode only; used by function
	// out_ncdf_annual)
	double crownarea_ageclass[OUTPUT_MAXAGECLASS];
	// plant crown area by age class (cohort/individual mode only; used by function
	// out_ncdf_annual)
	double fpc_ageclass[OUTPUT_MAXAGECLASS];
	// foliar projective cover (FPC) under full leaf cover as fraction of modeled area
	// by age class (cohort/individual mode only; used by function out_ncdf_annual)
	double boleht_ageclass[OUTPUT_MAXAGECLASS];
	// bole height, i.e. height above ground of bottom of crown cylinder (m)
	// by age class (cohort/individual mode only; used by function out_ncdf_annual)
	double live_cohorts[OUTPUT_MAXAGECLASS];
	double csap_ageclass[OUTPUT_MAXAGECLASS];
	// cmass_sap by age class (cohort/individual mode only; used by function
	// out_ncdf_annual)
	double cheart_ageclass[OUTPUT_MAXAGECLASS];
	// cmass_heart by age class (cohort/individual mode only; used by function
	// out_ncdf_annual)
	double cleaf_ageclass[OUTPUT_MAXAGECLASS];
	// cmass_leaf by age class (cohort/individual mode only; used by function
	// out_ncdf_annual)
	double croot_ageclass[OUTPUT_MAXAGECLASS];
	// cmass_root by age class (cohort/individual mode only; used by function
	// out_ncdf_annual)
	double crepr_ageclass[OUTPUT_MAXAGECLASS];
	// cmass_repr by age class (cohort/individual mode only; used by function
	// out_ncdf_annual)

	//LMFire output variables - @KE added 2018
	double BBlive;
	// sum/mean across patches for burned live fuel by PFT (kgC/m2)
	double fkill;
	// sum/mean across patches for tree biomass killed (but not consumed) by fire (kgC/m2)
	double CKkill;
	// Due to crown damage: sum/mean across patches for tree biomass killed (but not consumed) by fire (kgC/m2)
	double taukill;
	// Due to cambial damange: sum/mean across patches for tree biomass killed (but not consumed) by fire (kgC/m2)
	double livefuel_1;
	double livefuel_10;
	double livefuel_100;
	double livefuel_1000;
	// sum/mean across patches of live fuel load in 1- 10- 100- and 1000- hour fuel classes (Kg DM m-2)
	double deadfuel_1;
	double deadfuel_10;
	double deadfuel_100;
	double deadfuel_1000;
	// sum/mean across patches of dead fuel load in 1- 10- 100- and 1000- hour fuel classes (Kg DM m-2)
