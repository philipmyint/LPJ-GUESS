...
xtring title;
vegmodetype vegmode;
firemodetype firemode; // added by weichao
int npatch;
...
enum {CB_NONE,CB_VEGMODE,CB_FIREMODE,CB_CHECKGLOBAL,CB_LIFEFORM,CB_LANDCOVER,CB_PHENOLOGY,CB_LEAFPHYSIOGNOMY,CB_SELECTION,	
	...
	CB_PATHWAY,CB_ROOTDIST,CB_EST,CB_CHECKPFT,CB_STRPARAM,CB_NUMPARAM,CB_WATERUPTAKE}; // modified by weichao (add CB_FIRRMODE)
...
void initsettings() {
	...
	npatch=1;
	vegmode=COHORT;
	firemode=LMFIRE;     // added by weichao
	run_landcover = false;
...
void plib_declarations(int id,xtring setname) {
	...
	case BLOCK_GLOBAL:
		...
		declareitem("firemode",&strparam,16,CB_FIREMODE,
			"Vegetation mode (\"LPJFIRE\", \"LMFIRE\",  \"LMFIRE_CF\", \"NOFIRE\")");	// added by weichao		
		...
		declareitem("patcharea",&patcharea,1.0,1.0e4,1,CB_NONE,
			"Patch area (m2)");
		declareitem("firepatch", &firepatch, 1.0, 1.0e8, 1, CB_NONE,
				"Fire patch area (m2)"); // LMFire @KE 2018
		...
		declareitem("diam_min", &diam_min, 0.0,1000,1, CB_NONE,
				"minimum value for thinning at diameter scale for trees"); // added by weichao
		declareitem("diam_max", &diam_max, 0.0,1000,1, CB_NONE,
				"maximum value for thinning at diameter scale for trees"); // added by weichao
		declareitem("cut_int", &cut_int, 1.0, 1.0e8, 1, CB_NONE,
				"cutting intervals. we set to 5, 10, 15, 20"); // added by weichao
		declareitem("thinning_only_once", &thinning_only_once, 1.0, CB_NONE,
				"whether to do forest thinning just once at first cutting year (1) or not (0)"); // added by weichao
		declareitem("shrub_remaining_frac", &shrub_remaining_frac, 0.0,1000,1, CB_NONE,
				"thinning remaining fraction for shrub biomass(0.0-1.0)"); // added by weichao
		declareitem("take_stem_out_of_ecosystem", &take_stem_out_of_ecosystem, 1.0, CB_NONE,
				"Whether to take stem out of ecosystem after management (0,1)"); // added by weichao
		declareitem("take_all_out_of_ecosystem", &take_all_out_of_ecosystem, 1.0, CB_NONE,
				"Whether to take all out of ecosystem after management (0,1)"); // added by weichao
		declareitem("prescribed_fire", &prescribed_fire, 1.0, CB_NONE,
				"Whether to do prescribed fire(1) or not (0)"); // added by weichao
		declareitem("dead_fuel_remaining", &dead_fuel_remaining, 0.0,1000,1, CB_NONE,
				"Dead fuel remaining fraction after prescribed fire"); // added by weichao	 			
	...
	case BLOCK_PFT:
		...
		// LMFire @KE 2019
		declareitem("CLs", &ppft->CLs, -100., 10., 1, CB_NONE,
        		"Crown length slope");
		declareitem("CLi", &ppft->CLi, -100., 10., 1, CB_NONE,
				"Crown length intercept");
		declareitem("BTs", &ppft->BTs, -100., 10., 1, CB_NONE,
				"Bark thickness slope");
		declareitem("BTi", &ppft->BTi, -100., 10., 1, CB_NONE,
				"Bark thickness intercept");
...
void plib_callback(int callback) {
	...
	// added by weichao	
	case CB_FIREMODE:  
		if (strparam.upper()=="LPJFIRE") firemode=LPJFIRE;
		else if (strparam.upper()=="LMFIRE") firemode=LMFIRE;
		else if (strparam.upper()=="LMFIRE_CF") firemode=LMFIRE_CF;
		else if (strparam.upper()=="NOFIRE") firemode=NOFIRE;
		else {
			sendmessage("Error",
				"Unknown fire mode (valid types: \"LPJFIRE\",\"LMFIRE\", \"LMFIRE_CF\", \"NOFIRE\")");
			plibabort();
		}
		break;		
	...
	case CB_CHECKGLOBAL:
		...
		if (!itemparsed("firemode")) badins("firemode"); // added by weichao		
		...
		if (run_landcover) {
			...
			if (!itemparsed("first_cutyear")) badins("first_cutyear"); //added by weichao
			if (!itemparsed("diam_min")) badins("diam_min"); //added by weichao
			if (!itemparsed("diam_max")) badins("diam_max"); //added by weichao
			if (!itemparsed("cut_int")) badins("cut_int"); //added by weichao
			if (!itemparsed("thinning_only_once")) badins("thinning_only_once"); //added by weichao
			if (!itemparsed("shrub_remaining_frac")) badins("shrub_remaining_frac"); //added by weichao
			if (!itemparsed("take_stem_out_of_ecosystem")) badins("take_stem_out_of_ecosystem"); //added by weichao
			if (!itemparsed("take_all_out_of_ecosystem")) badins("take_all_out_of_ecosystem"); //added by weichao
			if (!itemparsed("prescribed_fire")) badins("prescribed_fire"); //added by weichao
			if (!itemparsed("dead_fuel_remaining")) badins("dead_fuel_remaining"); //added by weichao
		}
		...
		if (vegmode==COHORT || vegmode==INDIVIDUAL) {
			...
			if (!itemparsed("firepatch")) badins("firepatch");
