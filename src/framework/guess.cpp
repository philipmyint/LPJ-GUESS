...
bool Patch::has_fires() const {
    ...
    return iffire && stand.landcover != CROPLAND && // revised by weichao turn on fire with management
...
bool Patch::has_disturbances() const {
	...
	return ifdisturb && stand.landcover != CROPLAND && // revised by weichao turn on disturbance with management
		(stand.landcover != PASTURE || disturb_pasture);
}
...
double Patch::cflux() {
	...
	cflux += fluxes.get_annual_flux(Fluxes::PRESCRIBED_FIRE); // added by weichao
...
void Individual::reduce_biomass(double mortality, double mortality_fire) {
	...
	if (!negligible(mortality)) {
		...
		// added by weichao
		if(cflux_fire > 0.0){
			dprintf("Attention: fire flux not from LMFire-------------------------");
		}
		// end of addition by weichao
