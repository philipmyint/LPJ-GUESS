...
// #include "indata.h"   // added by weichao
#include "LM_Fire.h"  // added by weichao
	...
	while (gc_itr != gridcell.end()) {
		...
		while (stand.isobj) {
			...
			// for (int p = 0; p < npft; p++) {
			    // if ((date.year - nyear_spinup == 10) && (date.day > 270)){
		         // dprintf("patch.pft[%d].litter_leaf = %f\n",p,patch.pft[p].litter_leaf);
		         // // dprintf("pft.id = %f\n",pft.id);
	            // }
			// }
			// Interception
			...
			if ((firemode == LMFIRE) || (firemode == LMFIRE_CF) && (date.year - nyear_spinup > -500)){ //&& date.year > 999 TESTING AUGUST
						// LM Fire occurrence && (date.year - nyear_spinup > 0)

				// if (stand.landcover == FOREST) {
					// lmfire(patch, stand, gridcell, pftlist);
				// }
			...
			// if ((firemode == LMFIRE) || (firemode == LMFIRE_CF)){ //&& date.year > 999 TESTING AUGUST
						// // LM Fire occurrence
				// // ListArray_id<Coord> gridlist;
				
				// lmfire(patch, stand, gridcell, pftlist); //@KE added, CHANGE when should this be called??
				// // Vegetation& vegetation = patch.vegetation;
				// // Individual& indiv = vegetation.getobj();
				// // if (isnan(indiv.fpc)){
		            // // dprintf("date.day = %f date.year = %f\n",date.day,date.year);
		...
		if (date.islastday && date.islastmonth) {
			...
			while (stand.isobj) {
			    ...
			    // for (int p = 0; p < npft; p++) {
			       // if ((date.year - nyear_spinup == 10) && (date.day > 270)){
		              // dprintf("patch.pft[%d].litter_sap = %f\n",p,patch.pft[p].litter_sap);
		         // // dprintf("pft.id = %f\n",pft.id);
	               // }
			    // }
