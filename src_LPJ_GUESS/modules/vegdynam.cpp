...
bool establish(Patch& patch, const Climate& climate, Pft& pft) {
	...
	// revised by weichao
	if (!patch.managed_this_year && (climate.mtemp_min20 < pft.tcmin_est ||
		...
		// Is this PFT already represented?
			...
			while (vegetation.isobj && !present) {
				...
				// if (indiv.pft.id==pft.id) {
					// present=true; // modified by weichao
		            ...
		            // patch.pft[pft.id].litter_heart = patch.pft[indiv.pft.id].litter_heart;
						...
						// dprintf("Vegetation----litter_leaf = %f litter_root = %f litter_sap = %f litter_heart = %f\n",patch.pft[pft.id].litter_leaf, 
						// patch.pft[pft.id].litter_root, patch.pft[pft.id].litter_sap, patch.pft[pft.id].litter_heart);
						// }
...
void establishment_guess(Stand& stand,Patch& patch) {
	...
	while (pftlist.isobj) {
		...
		if(establish_active_pfts_before_management)
			est_this_year = !patch.managed_this_year || !patch.plant_this_year && standpft.reestab;
...
void mortality_lpj(Stand& stand, Patch& patch, const Climate& climate, double fireprob) {
	...
	while (vegetation.isobj) {
		...
		if (indiv.pft.lifeform==TREE) {
			...
			else if ((firemode == LMFIRE) || (firemode == LMFIRE_CF)){
				fail("LMFIRE firemode only for cohort vegmode, please use LPJFIRE firemode when using population vegmode");
				mort_fire = 0.0; // added by weichao
			}
			else { //NOFIRE
				mort_fire = 0.0;
			}
...
// there are flux to atmosphere from brunt above-grround biomass in KE's codes, but they are in guess.cpp in this version (weichao)
			...
			if (patch.has_fires() && firemode == LPJFIRE)
				mort_fire=fireprob*(1.0-indiv.pft.fireresist);
			else mort_fire=0.0;
...
void mortality_guess(Stand& stand, Patch& patch, const Climate& climate, double fireprob) {
				...
				if (indiv.pft.lifeform==GRASS) {
					...
					if (ifstochmort) {
						...
						for (i=0;i<nindiv_prev;i++){
							// if(firemode == LMFIRE || firemode == LMFIRE_CF){
								// indiv.pft.fireresist = 1.0;
							// }
							if (randfrac(stand.seed)>indiv.pft.fireresist && firemode == LPJFIRE) nindiv--;
						}
	...
	//if (firemode == LMFIRE) Mortality calculated and vegetation state variables updated in lmfire()
...
///////////////////////////////////////////////////////////////////////////////////////
// IMPOSED HISTORIC DISTURBANCE @KE
// Generic patch-destroying disturbance with a prescribed occurrence based on historic
// disturbance data.
// Internal function - do not call from framework

void histdisturbance(Patch& patch, bool ifdisturbyr) {
	//dprintf("HISTDISTURBANCE CALLED\n"); //@KE
	// DESCRIPTION
	...
	// INPUT PARAMETER
	// ifdisturbyr = if there was a disturbance this year in historical data
