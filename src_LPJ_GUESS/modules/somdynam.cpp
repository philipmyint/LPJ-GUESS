...
void som_dynamics_lpj(Patch& patch, bool tillage) {
	...
	for (p=0;p<npft;p++) {	//NB. also inactive pft's
	    ...
	    // if (date.year - nyear_spinup == 100 && date.day == 240){
            // dprintf("----somdynam.cpp----litter_leaf = %f litter_root = %f litter_sap = %f litter_heart = %f litterr_wood = %f\n",patch.pft[p].litter_leaf, 
	        // patch.pft[p].litter_root, patch.pft[p].litter_sap, 
	        // patch.pft[p].litter_heart, patch.pft[p].litter_wood);
	   // }
	}
...
void transfer_litter(Patch& patch) {
	...
	bool drop_leaf_root_litter = (lat >= 0.0 && date.month == 0) || (lat < 0.0 && date.month == 6) || patch.is_litter_day; // added by weichao, should we change the is_litter_day to true?
	...
	while (patch.pft.isobj) {
		...
		if (drop_leaf_root_litter) {
			...
			// if (date.year - nyear_spinup == 100){
			    ...
			    // dprintf("----somdynam.cpp--beforeRemove--date.day = %d leaf_littter = %f\n",date.day,leaf_littter);
		    // }
			...
			// if (date.year - nyear_spinup == 100){
			    ...
			    // dprintf("----somdynam.cpp--beforeRemove--date.day = %d root_littter = %f\n",date.day,root_littter);
		    // }
		...
		// added by weichao
			// if (date.year - nyear_spinup == 100){
				...
				// // dprintf("----somdynam.cpp--beforeRemove--date.day = %d litter_leaf = %f litter_root = %f litter_sap = %f litter_heart = %f litter_wood = %f\n",date.day,patch.pft[pft.id].litter_leaf, \
				// // patch.pft[pft.id].litter_root, patch.pft[pft.id].litter_sap, patch.pft[pft.id].litter_heart, patch.pft[pft.id].litter_wood);
				// dprintf("----somdynam.cpp--beforeRemove--date.day = %d litter_wood = %f\n",date.day,patch.pft[pft.id].litter_wood);
		    // }
			...
			// if (date.year - nyear_spinup == 100 && date.day == 240){
				...
				// dprintf("----somdynam.cpp--AfterRemove--litter_leaf = %f litter_root = %f litter_sap = %f litter_heart = %f litterr_wood = %f\n",patch.pft[pft.id].litter_leaf, 
				// patch.pft[pft.id].litter_root, patch.pft[pft.id].litter_sap, 
				// patch.pft[pft.id].litter_heart, patch.pft[pft.id].litter_wood);
		    // }
		}
		...
		if (pft.pft.lifeform == TREE && date.dayofmonth == 0) {
			...
			if (!negligible(litter_heart)) {
			    ...
			    // if (date.year - nyear_spinup == 100){
				    ...
				    // dprintf("----somdynam.cpp--beforeRemove--date.day = %d wood_littter = %f\n",date.day,wood_littter);
		        // }
